...
namespace TextRendering
{
    class TextRenderer
    {
        ...
        public Bitmap RenderedText(string message, int xadvance, int percent)
        {
            Bitmap source = new Bitmap(720, 480);
            float x = 0;
            int y = 0;
            ...
            foreach(var character in message)
            {
                ...
                var kvp = CustomChars.Where(x => x.Value == character);
                if (kvp.Count() > 0) 
                    codePoint = kvp.First().Key;

                source = BitmapOnBitmap(source, CharBitmap(codePoint), (int)x, y, scale);
                x += _baseGameScale * xAdvanceRatio * scale;
            }
            return source;
        }
        ...
        private static Dictionary<byte, char> CustomChars = new Dictionary<byte, char>()
        {
            [0xA8] = 'Á',
            [0xA9] = 'á',
            [0xAA] = 'À',
